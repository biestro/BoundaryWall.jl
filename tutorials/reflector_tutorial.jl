...
X   = collect(0:1.0:4) # centers
T   = zeros(length(X)) # angle
len = 8.0
barrier = [createEllipse(0.0, len, LinRange(-pi/2,pi/2,N), th, SVector(x0, 0.0)) for (x0, th) in zip(X, T)]
x  = vcat(getindex.(barrier, 1)...)
y  = vcat(getindex.(barrier, 2)...)
xm = vcat(getindex.(barrier, 3)...)
ym = vcat(getindex.(barrier, 4)...)
...
STRENGTH = ones(length(xm)÷length(X))
POTENTIAL_STRENGTH = kron([-20,-10,-20,-10, -20],STRENGTH)
scatter(x,y, color=POTENTIAL_STRENGTH)
...
ϕ          = 140.0
waveVector = 5*SVector(cosd(ϕ), sind(ϕ)) 
@time wave = BWM.boundaryWallWave(waveVector, (k,r)->BWM.gaussianWave(k,r - SVector(0.0, 2.0), 10.0; abstol=1e-8), x, y, xm, ym, XDOM, YDOM, SIGMA, ds, rij, length(ds), N, banded, POTENTIAL_STRENGTH);
...
begin
  ...
  [lines!(ax1, createEllipse(0.0, len, LinRange(-pi/2,pi/2,N), th, SVector(x0, 0.0))[1:2]..., linestyle=:solid) for (x0, th) in zip(X, T)]
  [lines!(ax2, createEllipse(0.0, len, LinRange(-pi/2,pi/2,N), th, SVector(x0, 0.0))[1:2]..., linestyle=:solid) for (x0, th) in zip(X, T)]
  # [lines!(ax2, createEllipse(0.0, 7.0, LinRange(-pi/2,pi/2,N), pi/4, SVector(x0, 0.0))[1:2]...,color=(:black,1.0)) for x0 in centers[1]]
  # [lines!(ax, createEllipse(0.0, 5.0, LinRange(-pi/2,pi/2,N), th, SVector(x0, 0.0))[1:2]..., color=:white) for (x0, th) in zip([0.0, 2.0], [pi/6, pi/4])]
  # xlims!(ax[1], xdom[1], xdom[end])
  # ylims!(ax[1], ydom[1], ydom[end])
