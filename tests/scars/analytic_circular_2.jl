...
#    return omegaN(n, p)*GSL.sf_bessel_Jn(n,k*R)/sf_hankelh1_n(n,k*R)
    return omegaN(n, p)*besselj(n,k*R)/hankelh1(n,k*R)
end

# valFun(_k::Float64, _r::Float64,_ω::ComplexF64) = besselj(0,_k*_r)*(1+_ω)

"""
 Uses SpecialFunctions
"""
function circleWaveBase(k::Union{Float64, BigFloat}, 
    ...
    _params = [SVector(im^n,omegaN(n,p),cos(n*(r[2]-pi/2)),uN(n,p,k,R)) for n in 1:_N_sum]
    # _Jn = [besselj(_n,k*r[1]) for _n in 1:_N_sum]
    _Jn = besselj.(1:_N_sum,Ref(k*r[1]))    
    if r[1]<R
        # _val= GSL.sf_bessel_J0(k*r[1])*(1+omegaN(0,γ,σ,k,R))
        _val= besselj0(k*r[1])*(1+omegaN(0,p))
    ...
    # # else
    _val = besselj0(k*r[1]) + uN(0,p,k,R) * (hankelh1(0,k*r[1]))
    # _Yn  = [bessely(_n,k*r[1]) for _n in 1:_N_sum]
    _Yn  = bessely.(1:_N_sum,Ref(k*r[1]))
    _sum = sum(
...
# 
# polar_svec = [m_rot * SVector(_x...) for _x in COORDS]
...
wave_scars= zeros(ComplexF64, length(polar_coords))
wave_anal = zeros(ComplexF64, length(polar_coords))
...
_m_prime      = quantumNumberM0 + turningPointsQ*_K
_n_prime      = quantumNumberN0 - windingsP*_K
# _m_rot        = Meshes.Angle2d(pi/(2_m_prime))
#

# _polar_svec   = [_m_rot * SVector(_x...) for _x in COORDS]
# XROT, YROT    = first.(_polar_svec), last.(_polar_svec)
_polar_rot    = [SVector(_r[1], _r[2]-pi/(2*_m_prime)) for _r in polar_coords]
# _polar_rot    = [car2pol(_z...) for _z in _polar_svec]

_bessel_roots = _k = FunctionZeros.besselj_zero(_m_prime, _n_prime)/R #

println("m: $_m_prime\t n:$_n_prime\t  nWave number: $_K \t $_bessel_roots  ")
...
_phase = exp(im*_K*turningPointsQ*phi_0)
global wave_scars += _phase*_wave*1/sqrt(2*nearlyDegenerateM+1)
push!(wave, _wave)

println("Calculating analytic wave...")
_wave_even = [circleWaveBase(_k/R, Inf,SIGMA,R,_r,_m_prime, 0.0, nSum) for _r in polar_coords]
_wave_odd  = [circleWaveBase(_k/R, Inf,SIGMA,R,_r,_m_prime, 0.0, nSum) for _r in _polar_rot]

global wave_anal += sqrt(0.5)*(_wave_even + im*_wave_odd)*_phase*1/sqrt(2*nearlyDegenerateM+1)
push!(wave_even, _wave_even)
push!(wave_odd, _wave_odd)
...
let fig = Figure()
    ...
    heatmap!(ax[1], xdom, ydom, reshape(abs.(wave), nx, ny), colormap=:linear_kbgyw_5_98_c62_n256, interpolate=true)
    heatmap!(ax[2], xdom, ydom, reshape(abs.(wave_maioli), nx, ny), colormap=:linear_kbgyw_5_98_c62_n256, interpolate=true, colorrange=(0,0.5))
