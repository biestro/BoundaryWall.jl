...
#   [
#     [
#       lines!(ax, getindex.(Ref(GeometryUtils.createCircle(R, LinRange(-pi, pi, 150), c)), (1, 2))..., color=(:black,1.0), linewidth=1.5)
#       for c in centers
#     ] 
#     for ax in ax
#   ]
...
let 
  ...
  ax = [Axis(fig[1,1], title="Ex"), 
        ...
        Axis(fig[2,1], title="Ez"),
        Axis(fig[2,2], title="Etot")]
  viz!(ax[1], _abaqus_mesh, color=abs2.(Ex), colormap=:amp)
  viz!(ax[2], _abaqus_mesh, color=abs2.(Ey), colormap=:amp)
  viz!(ax[3], _abaqus_mesh, color=abs2.(Ez), colormap=:amp)
  viz!(ax[4], _abaqus_mesh, color=abs2.(hypot.(Ex, Ey, Ez)), colormap=:amp)
  [ax.aspect=DataAspect() for ax in ax]
  fig
end
...
let fig=Figure(fontsize=14,size=(700,600), theme=theme_latexfonts())
  ...
  # save("./images2/stokes.png", fig, px_per_unit = 3)
  fig
end