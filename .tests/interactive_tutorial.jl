...
begin
  ...
  INDICES = sort([55,56,57,58,59,60,51,40,41,42,43,44,45,31,22,21,20,19])
...
begin
  ...
  @inline function double_beam(k,r, w)
    BWM.gaussianWave(k,r - SVector(-10.0, 6.0), w; abstol=1e-3) + 
    BWM.gaussianWave(k,r - SVector(-10.0, -6.0), w; abstol=1e-3)
  end

  @inline function wave_function(_freq::Float64, _kvec::SVector{2, Float64}, _width::Float64)
    return reshape(
      BWM.boundaryWallWave(_freq * _kvec, (k,r) -> double_beam(k,r, _width), x, y, xm, ym, XDOM, YDOM, SIGMA, ds, rij, length(ds), N, banded, POTENTIAL_STRENGTH),
      NDOM, NDOM
    )
  end
